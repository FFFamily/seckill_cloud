package com.tutu.gateway.filtter;
...
import com.tutu.gateway.config.IgnoreUrlsConfig;
...
public class MyGlobalFilter implements GlobalFilter, Ordered {
    ...
    public Mono<Void> filter(ServerWebExchange exchange, GatewayFilterChain chain) {
        ...
        // 用户Id =》 作为Redis中的存储Key
        String userKey = RedisKey.getUserKey((String) userInfo.get("userId"));
        // 判断是否在Redis中，是否已登录
        if (!redisUtil.hasKey(userKey)) {
            return error(exchange, "登录状态已过期，请重新登录",null);
        }
        // 判断Token是否一致
        if (!DigestUtil.md5Hex(realToken).equals(redisUtil.get(userKey).toString())){
            return error(exchange, "当前用户令牌非法",null);
        }
        // 添加到请求头中
        ...
        // 放行重构之后的request
        ServerWebExchange requestBuild = exchange.mutate().request(mutate.build()).build();
